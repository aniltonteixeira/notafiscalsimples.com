    ...
    <div className="flex min-h-screen bg-[#F7F9FC]">
      ...
      <aside className={`fixed top-0 left-0 h-full w-64 bg-white border-r z-50 transition-transform md:hidden ${menuAberto ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <Image src="/logonfs.svg" alt="Logo" width={130} height={40} />
          <button onClick={() => setMenuAberto(false)}>
            <X size={24} />
          </button>
        </div>
        <nav className="p-4 space-y-2 text-sm">
          <Link href="/admin/empresas" className="flex items-center gap-2 p-2 rounded hover:bg-blue-50" onClick={() => setMenuAberto(false)}>
            <Building2 size={18} /> Empresas
          </Link>
          <Link href="/admin/usuarios" className="flex items-center gap-2 p-2 rounded hover:bg-blue-50" onClick={() => setMenuAberto(false)}>
            <Users2 size={18} /> Usuários
          </Link>
          <Link href="/admin/notas" className="flex items-center gap-2 p-2 rounded hover:bg-blue-50" onClick={() => setMenuAberto(false)}>
            <FileText size={18} /> Notas fiscais
          </Link>
          <Button
            variant="ghost"
